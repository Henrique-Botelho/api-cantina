...
const comprasController = {
  ...
  excluirComprasPorCliente: async (req, res) => {
    ...
    const queryVerificaCompras = 'SELECT * FROM compras WHERE id_cliente=?';
    ...
    try {
      ...
      let compras = await Promise.all(
        clientes.map(cliente => pool.query(queryVerificaCompras, [cliente.id]))
      );
      ...
      for (const compra of compras) {
        const resultado = await pool.query(queryExcluirCompras, [compra.id_cliente]);
