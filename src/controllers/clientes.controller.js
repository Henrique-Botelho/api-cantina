...
const clientesController = {
    ...
    atualizaCliente: async (req, res) => {
        ...
        if (!id || (!nome && !numero)) {
          // Verificando se o id foi informado e se ao menos um dos campos (nome ou número) foi informado.
          return res.status(400).json({ status: 400, message: 'É necessário informar o id do cliente e pelo menos um campo a ser atualizado.' });
        }
      
        // Verificando se o cliente com o id informado existe.
        ...
        // Atualizando o nome e/ou o número do cliente.
        const queryAtualizaCliente = 'UPDATE clientes SET nome = ?, numero = ? WHERE id = ?';
        try {
          await pool.query(queryAtualizaCliente, [nome, numero, id]);
